...
/*   Color gray = Color(200, 200, 200);
    ...
    renderer.render();
*/ 
    Color gray = Color(50.0f, 60.0f, 80.0f);
    Scene scene;
    Surface base = Surface(gray, 1.0f, 5.0f);
    auto sphere = std::make_unique<Sphere>(2.0f, point3D(0.0f, 0.0f, 5.0f), base, 5.0f, 5.0f);
    Camera camera = Camera(point3D(0,0,-10), Z_AXIS, 90.0f, 4.0f /3.0f, 1080);
    auto light = std::make_unique<Point_Light>(point3D(5.0f, 5.0f, 0.0f), 30.0f);    
    scene.add_light(std::move(light));
    scene.add_object(std::move(sphere));
    Renderer renderer = Renderer(camera, scene);
    renderer.render();
